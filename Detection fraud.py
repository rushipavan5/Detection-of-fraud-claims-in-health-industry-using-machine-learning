...
import ClientRegister_Model,insurance_claim_frauds,detection_ratio,detection_accuracy
...
returnrender(request,
             'SProvider/View_Predicted_Health_Insurance_Claim_Fraud_Type_Ratio.html', {'objs': obj})
...
def charts(request,chart_type):
    ...
    return render(request,"SProvider/charts.html",
                  {'form':chart1, 'chart_type':chart_type})
...
def View_Predicted_Health_Insurance_Claim_Fraud_Type(request):
    ...
    return
render(request,'SProvider/View_Predicted_Health_Insurance_Claim_Fraud_Type.html',
       {'list_objects': obj})
def likeschart(request,like_chart):charts =detection_accuracy.objects.values('names').annotate(dcount=Avg('ratio'))
...
detection accuracy. objects. Create (names="SVM", ratio=svm_acc)
